...
function MiCuenta() {
  ...
  const handleLogin = async () => {
    ...
    } catch (error) { alert("Error: El servidor está apagado"); }
  };
...
<TextField fullWidth label="Contraseña" name="password" margin="normal" variant="outlined" onChange={handleChange} type={showPassword ? "text" : "password"}  
  helperText={modo === "registro" ? "Mínimo 8 caracteres, una mayúscula y un número" : ""}InputProps={{endAdornment: (
...
/>
{modo === "login" ? (
  <Button variant="contained" fullWidth size="large" sx={{ mt: 3, bgcolor: '#00b7ff' }} onClick={handleLogin}>Entrar</Button>
) : (
  <Button variant="contained" color="success" fullWidth size="large" sx={{ mt: 3 }} onClick={handleRegistro}>Registrarme ahora</Button>
)}

        <Button fullWidth color="white" sx={{ mt: 3, textTransform: 'none' }} onClick={() => setModo(modo === "login" ? "registro" : "login")}>
